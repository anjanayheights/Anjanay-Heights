...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    const meta = md.meta || {};
    const queue = (ld.leads || []).map((lead: any) => {
      const m = meta[lead.id] || {};
      const s = String(m.status || '').toLowerCase();
      if (s === 'closed' || s === 'lost' || !cleanPhone(lead.phone)) return null;
      const last = m.lastContactAt || m.last_contact_at || lead.updated_at || lead.created_at;
      const age = daysSince(last); const priority = String(m.priority || '').toLowerCase();
      const score = Number(m.score || lead.score || 0); let action = 'MONITOR'; let urgency = 0; let message = '';
  ...
  } catch (e) { return res.status(500).json({ error: e instanceof Error ? e.message : 'Unable to build re-engagement queue' }); }
}
