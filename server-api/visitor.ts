 ...
 try{
  if(req.method==='GET'){
   const [vr,pr,dr]=await Promise.all([
    ...
    fetch(URL+'/rest/v1/website_visitor_days?select=day,visitor_id&order=day.desc&limit=5000',{headers:headers()})
   ]);
   if(!vr.ok||!pr.ok||!dr.ok)throw new Error('visitor analytics read failed');
   const rows:any[]=await vr.json(), views:any[]=await pr.json(), days:any[]=await dr.json(), now=Date.now(), cut=now-90000;
   const today=istDay();
   const active=rows.filter(x=>Date.parse(x.last_seen)>=cut).length;
   const todayVisitors=days.filter(x=>x.day===today).length;
   const dayCounts=new Map<string,number>(); days.forEach(x=>dayCounts.set(x.day,(dayCounts.get(x.day)||0)+1));
   const dailyVisitors=[...dayCounts.entries()].sort((a,b)=>b[0].localeCompare(a[0])).slice(0,14).map(([day,visitors])=>({day,visitors}));
   ...
   return send(res,200,{totalViewers:rows.length,activeVisitors,todayVisitors,dailyVisitors,topPages,topProperties});
  }
  if(req.method==='POST'){
   let id=cookie(req),isNew=false;if(!id){id=crypto.randomUUID();isNew=true}
   const path=String(req.headers?.['x-page-path']||req.headers?.referer||'/').replace(/^https?:\/\/[^/]+/,'')||'/';
   const propertyMatch=path.match(/^\/property\/([^/?#]+)/);const propertyId=propertyMatch?decodeURIComponent(propertyMatch[1]):null;
   const now=new Date().toISOString(), day=istDay();
   const r=await fetch(URL+'/rest/v1/website_visitors?on_conflict=visitor_id',{method:'POST',headers:{...headers(),Prefer:'resolution=merge-duplicates,return=minimal'},body:JSON.stringify({visitor_id:id,last_seen:now,last_path:path,...(isNew?{first_path:path}:{})})});
   if(!r.ok)throw new Error('visitor write '+r.status);
   const dr=await fetch(URL+'/rest/v1/website_visitor_days?on_conflict=day,visitor_id',{method:'POST',headers:{...headers(),Prefer:'resolution=merge-duplicates,return=minimal'},body:JSON.stringify({day,visitor_id:id,first_seen:now,last_seen:now,last_path:path})});
   if(!dr.ok)throw new Error('daily visitor write '+dr.status);
   const pv=await fetch(URL+'/rest/v1/website_page_views',{method:'POST',headers:{...headers(),Prefer:'return=minimal'},body:JSON.stringify({visitor_id:id,path,property_id:propertyId})});
   if(!pv.ok)throw new Error('page view write '+pv.status);
