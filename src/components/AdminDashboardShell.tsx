...
export default function AdminDashboardShell() {
  ...
  return <div className="min-h-screen bg-[#F5F7FA] pb-20 md:pb-0">
    ...
    <div className="max-w-7xl mx-auto px-4 md:px-8 pt-5"><div className="bg-white rounded-2xl border p-3 mb-5"><div className="flex gap-2 overflow-x-auto pb-1">{nav.slice(0,8).map(([icon,label,url]) => <button key={url} onClick={()=>{window.location.href=url}} className="shrink-0 rounded-xl bg-[#F5F7FA] px-3 py-2 text-xs font-semibold text-[#1A365D]">{icon} {label}</button>)}</div><div className="flex gap-2 overflow-x-auto pt-2">{nav.slice(8).map(([icon,label,url]) => <button key={url} onClick={()=>{window.location.href=url}} className="shrink-0 rounded-xl border px-3 py-2 text-xs font-semibold text-slate-700">{icon} {label}</button>)}</div></div></div>
