...
import { Eye, RefreshCw, Users, BarChart3, CalendarDays } from 'lucide-react';
...
export default function WebsiteViewerCard() {
  ...
  return <section className="max-w-7xl mx-auto px-4 md:px-8 pt-6">
    <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
      ...
      {data && <div className="mt-5 grid md:grid-cols-2 gap-4">
        <div className="rounded-xl border p-4"><p className="text-sm font-semibold text-slate-700 mb-2">Top Pages</p>{(data.topPages||[]).length?(data.topPages||[]).map((x:any)=><div key={x.path} className="flex justify-between text-sm py-1"><span className="truncate pr-3 text-slate-600">{x.path}</span><span className="font-semibold text-[#1A365D]">{x.views}</span></div>):<p className="text-xs text-slate-400">No page views yet.</p>}</div>
        <div className="rounded-xl border p-4"><p className="text-sm font-semibold text-slate-700 mb-2">Top Properties</p>{(data.topProperties||[]).length?(data.topProperties||[]).map((x:any)=><div key={x.id} className="flex justify-between text-sm py-1"><span className="truncate pr-3 text-slate-600">{x.name}</span><span className="font-semibold text-[#1A365D]">{x.views}</span></div>):<p className="text-xs text-slate-400">No property-page views yet.</p>}</div>
        <div className="rounded-xl border p-4 md:col-span-2"><div className="flex items-center gap-2 mb-2"><CalendarDays size={17} className="text-[#1A365D]"/><p className="text-sm font-semibold text-slate-700">Daily Visitor History (IST)</p></div>{(data.dailyVisitors||[]).length?(data.dailyVisitors||[]).map((x:any)=><div key={x.day} className="flex justify-between text-sm py-1 border-b last:border-0 border-slate-100"><span className="text-slate-600">{x.day}</span><span className="font-semibold text-[#1A365D]">{x.visitors}</span></div>):<p className="text-xs text-slate-400">History will appear as visitors arrive.</p>}</div>
      </div>}
    </div>
  </section>;
}